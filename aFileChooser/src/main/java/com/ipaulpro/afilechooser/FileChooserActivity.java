...
public class FileChooserActivity extends FragmentActivity implements
    ...
    public FileChooserActivity() {
        if (EXTERNAL_BASE_PATH.startsWith("/storage/")) {
            EXTERNAL_BASE_PATH = "/storage/";
        }
    }
    ...
    // to call when the user long selects a file or directory
    public void onFileLongSelected(File file) {
        if (file != null) {
            if (file.isDirectory()) {
                //options menu right click style for folders
            } else {
                //options menu right click style for files
            }
        } else {
            Toast.makeText(FileChooserActivity.this, R.string.error_selecting_file,
                    Toast.LENGTH_SHORT).show();
